...
参与规则：
    ...
    代码规范：
      1. 除特殊约定以外，所有类、函数、结构、枚举等，全部包含在 namespace Tianchi { ... } 中。
      2. 目录、文件名、类名等，均不设特约前缀名。
      3. 所有成员变量名前缀为 m_ 。
      4. 方法名的命名方式参照 Qt 标准，首字母小写，其它单词首字母大写，中间不含下划线（_)。
      5. 类成员的 get 方法不含 get 前缀，但 set 方法全部包含 set 前缀。
      6. 除头文件防重复包含的预定义名外，其它预编译条件禁止使用 #ifndef ，请改为 #ifdef 或 #if defined(...)
      7. 允许参与者在源码的注释部分包括自己的名字和简介。
      8. 对为 public 接口进行用途和用法说明，共享库最终使用 doxygen 输出使用文档，请遵循 doxygen 的语法要求。
      9. 源码发布格式统一采用 AStyle 方式整理，对参与人员不作强制要求，但请尽量保持代码有较高可读性。
     10. 《天池》核心团队有权在正式版发布时，修改、删除其中的代码，并且不对更新者进行说明或解析。
