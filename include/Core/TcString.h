...
public:
    ...
    /// @brief 装入一个文本文件的全部内容
    /// @param [out] lines 返回的字符串容器
    /// @param [in] fileName 路径文件名
    /// @return true 操作成功
    static bool load(QStringList& lines, const QString& fileName, const char* codec="UTF8");

    /// @brief 保存当前内容到一个文本文件中
    /// @param [in] lines 需要保存的文本内容
    /// @param [in] fileName 路径文件名
    /// @return true 操作成功
    static bool save(const QStringList& lines, const QString& fileName, const char* codec="UTF8");


    /// @brief 装入一个文本文件的全部内容
    /// @param [in] fileName 路径文件名
    /// @return true 操作成功
    inline bool load(const QString& fileName, const char* codec="UTF8")
    {
        return load(*this, fileName, codec);
    }
    /// @brief 保存当前内容到一个文本文件中
    /// @param [in] fileName 路径文件名
    /// @return true 操作成功
    inline bool save(const QString& fileName, const char* codec="UTF8")
    {
        return save(*this, fileName, codec);
    }
