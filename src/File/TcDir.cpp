...
QStringList TcDir::findFiles(const QString &path, const QStringList &nameFilters)
{
    QStringList files;
...
int TcDir::copyDirs(const QString &fromPath, const QString &toPath, const QStringList &nameFilters, bool overWrite)
{
    resetCopyFileCount();
...
int TcDir::copyFiles(const QStringList &files, const QString &toPath, bool overWrite)
{
    int size = files.size();
    int count = 0;
